...
# **Introduction**
lkadjklajkldjak;jdk;jak;dj;ajdjajdajdljajdljaddjkl
...
## GitHub is not a backup because it only keeps records of my file rather than a copy"
```
...
data_clean <- select(data_no_NA,-loc,-comment)

# Check spelling in species and treatment but also generate a summary table
table(data_clean$species)
table(data_clean$treatment)

sl_mean <- mean(data_clean$sl)
activity_mean <- mean(data_clean$activity)
sl_se <- var(data_clean$sl)
activity_se <- var(data_clean$activity)
acantho_control <- sum(data_clean %>% filter(species == "acantho", treatment == "CO2"))
