...
data_clean <- data_no_irrelevant_columns %>% group_by(species, treatment) %>% mutate(sample_size = length(sl), mean_sl = mean(sl), standard_error_sl = var(sl), mean_activity = mean(activity), standard_error_activity = var(activity))
...
plot_ambon <- ggplot(data_clean%>%filter(species=="ambon"),aes(x=treatment,y=mean_activity, fill=treatment))+annotation_raster(pic_acantho, -Inf, Inf, -Inf, Inf)+geom_col(alpha = 0.6)+ggtitle("mean activity of ambon")
ggsave('./output/figures/mean activity of ambon.png', )

## chromis
pic_chromis <- readPNG('./pics/chromis.png')
plot_chromis <- ggplot(data_clean%>%filter(species=="chromis"),aes(x=treatment,y=mean_activity, fill=treatment))+annotation_raster(pic_acantho, -Inf, Inf, -Inf, Inf)+geom_col(alpha = 0.6)+ggtitle("mean activity of chromis")
ggsave('./output/figures/mean activity of chromis.png', )

## humbug
pic_humbug <- readPNG('./pics/humbug.png')
plot_humbug <- ggplot(data_clean%>%filter(species=="humbug"),aes(x=treatment,y=mean_activity, fill=treatment))+annotation_raster(pic_acantho, -Inf, Inf, -Inf, Inf)+geom_col(alpha = 0.6)+ggtitle("mean activity of humbug")
ggsave('./output/figures/mean activity of humbug.png', )

## lemon
pic_lemon <- readPNG('./pics/lemon.png')
plot_lemon <- ggplot(data_clean%>%filter(species=="lemon"),aes(x=treatment,y=mean_activity, fill=treatment))+annotation_raster(pic_acantho, -Inf, Inf, -Inf, Inf)+geom_col(alpha = 0.6)+ggtitle("mean activity of lemon")
ggsave('./output/figures/mean activity of lemon.png', )

## whitedams
pic_whitedams <- readPNG('./pics/whitedams.png')
plot_whitedams <- ggplot(data_clean%>%filter(species=="whitedams"),aes(x=treatment,y=mean_activity, fill=treatment))+annotation_raster(pic_acantho, -Inf, Inf, -Inf, Inf)+geom_col(alpha = 0.6)+ggtitle("mean activity of whitedams")
...
```{r, answer6}
## Maybe it is better if we track those PNG files because they are the source, like fundamental material, of what the previous code are using and also when working with others, others can see them too.
```
